...
void indy() {  // (indirect),Y
  ...
  eahelp2 = (eahelp & 0xFF00) | ((eahelp + 1) & 0x00FF);  // zero-page
                                                          // wraparound
  ea = (uint16_t)read6502(eahelp) | ((uint16_t)read6502(eahelp2) << 8);
  startpage = ea & 0xFF00;
  ea += (uint16_t)y;
...
void php() { push8(cpustatus | FLAG_BREAK); }
...
void plp() { cpustatus = pull8() | FLAG_CONSTANT; }
